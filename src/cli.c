...
static int cstow_cli_default(struct cstow_cli_options *options){
    ...
    if(getcwd(cwd, sizeof(cwd)) == NULL){
        cstow_error_set(&cstow_error, CSTOW_ERR_INTERNAL, errno, "cli", "cli default", NULL, NULL);
        return -1;
    }
...
int cstow_cli_flags_handle(int argc, char *argv[], struct cstow_cli_options *options){
    ...
    while((opt = getopt_long(argc, argv, ":hs:d:Dvn", long_options, NULL)) != -1){
        switch(opt){
            ...
            break;
            case '?':
                cstow_error_set(&cstow_error, CSTOW_ERR_INVALID_OPT, 0, "cli", "options handle", NULL, NULL);
                return -1;
            break;
            case ':':
                cstow_error_set(&cstow_error, CSTOW_ERR_MISSING_VAL, 0, "cli", "options handle", NULL, NULL);
                return -1;
            break;
        }
    }
